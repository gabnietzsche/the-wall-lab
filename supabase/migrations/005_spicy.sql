-- Migration 005 — Wave "spicy"
--
-- Modifiche:
-- 1. Durata partita: 60s effettivi (era 100s). ends_at = playing_from + 60s = started_at + 63s
-- 2. Nuovo bonus 'x2-coins': per 6s ogni moneta raccolta vale doppio
-- 3. Skin del giocatore: lobby.skin + games.player1_skin + games.player2_skin
-- 4. Sudden death: games.overtime_until — 15s extra al pareggio, vince chi rompe per primo un mattone con contenuto
-- 5. Combo Streak: player_states.streak — 3 mattoni di fila con contenuto utile → +1 moneta bonus
-- 6. Muro più randomico: 16-22 monete + 4-7 powerup (mix bonus/malus random)
...
alter table public.games
  add column if not exists player1_skin text,
  add column if not exists player2_skin text,
  add column if not exists overtime_until timestamptz;

alter table public.player_states
  add column if not exists streak int not null default 0;

-- Default tempi: 3s countdown + 60s gioco effettivo
alter table public.games
  alter column ends_at set default (now() + interval '63 seconds');

-- =========================================================================
-- View games_public — espongo skin + overtime_until al client
-- =========================================================================
drop view if exists public.games_public;
create view public.games_public with (security_invoker = on) as
  select id, status, started_at, ends_at, playing_from, overtime_until,
         player1_nick, player2_nick, player1_skin, player2_skin,
...
-- =========================================================================
-- _generate_wall: 16-22 monete, 4-7 powerup random, mix bonus/malus libero
-- =========================================================================
...
declare
  ...
  v_pup_count int;
begin
  -- Random per ogni partita
  v_coins_count := 16 + floor(random() * 7)::int;   -- 16..22
  v_pup_count   := 4  + floor(random() * 4)::int;   -- 4..7
  if v_coins_count + v_pup_count > 25 then
    v_pup_count := 25 - v_coins_count;
  end if;

  -- Powerup random dal pool (senza vincoli su bonus/malus)
  v_powerups := (
  ...
  -- Distribuzione: prime v_coins_count posizioni = coin
  for v_i in 1..v_coins_count loop
    v_contents[v_positions[v_i] + 1] := 'coin';
  end loop;
  -- Successive v_pup_count = powerup
...
-- =========================================================================
-- _create_game: ora accetta skin e usa il pool con x2-coins
-- =========================================================================
...
declare
  v_pool text[] := ARRAY[
    ...
    'scalpello','gabbia','doppia-gabbia','fantasma'
  ];
  v_gen jsonb;
  v_powerups text[];
  v_game_id uuid;
...
-- =========================================================================
-- request_match: ora accetta anche la skin del giocatore
-- =========================================================================
...
-- =========================================================================
-- hit_brick — gestione x2-coins, overtime, streak, e cooldown 1s
-- =========================================================================
...
begin
  ...
  -- Carico partita (senza filtro ends_at per gestire l'overtime)
  ...
  if v_now > v_game.ends_at then
    if v_game.overtime_until is null then
      ...
      -- Overtime scaduto: pareggio definitivo
      update public.games set status = 'finished' where id = v_game.id;
      return jsonb_build_object('ok', false, 'error', 'finished');
    end if;
  end if;
  ...
  -- Effetti che modificano il colpo
  ...
  -- Mio lato già rotto: colpo perso
  if (v_player = 1 and v_brick.front_broken_at is not null) or
    ...
    -- streak invariata (colpo nel vuoto, non rottura)
    return jsonb_build_object('ok', true, 'my_side_broken_already', true, 'streak', v_state.streak);
  end if;

  -- Incremento hits sul mio lato
  if v_player = 1 then
  ...
  if v_my_hits >= v_hits_needed then
    ...
    if v_other_broken_at is not null then
      if v_now - v_other_broken_at < (v_window_ms || ' milliseconds')::interval then
        -- Collisione: rollback dei premi facili
        v_outcome := 'lost';
        v_brick.taken_by := 0;

        if v_content = 'coin' then
          -- Rollback: l'altro aveva preso 1 o 2 (a seconda del suo x2). Per semplicità sottraggo 1.
          if v_other = 1 then
        ...
        if v_content in ('pistola','scudo','raggi-x','scalpello','x2-coins') then
          v_other_eff := (
            with ord as (
              select e, ord
      ...
      -- L'altro lato è intatto: prendo io
      v_outcome := 'won';
      ...
      -- Overtime: chi rompe per primo un mattone con contenuto utile vince
      if v_in_overtime and v_content is not null and v_content <> 'empty' then
        v_overtime_winner := true;
      end if;
    end if;

    -- Combo Streak: applica solo se ho rotto io con contenuto utile
    if v_outcome = 'won' and v_content is not null and v_content <> 'empty' then
      v_new_streak := v_state.streak + 1;
      if v_new_streak >= 3 then
        v_streak_bonus := v_coin_value;  -- rispetta x2 attivo
        if v_player = 1 then
      ...
      -- lost / opp_taken / empty → reset
      v_new_streak := 0;
    end if;
  else
    -- Il mio lato non si è rotto ancora (solo crepa): streak invariata
  ...
  -- Overtime: chiusura partita su vittoria
